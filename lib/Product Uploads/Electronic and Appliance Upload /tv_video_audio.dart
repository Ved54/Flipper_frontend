...
class TVAudioVideo extends StatefulWidget {
  const TVAudioVideo({super.key});

  @override
  State<TVAudioVideo> createState() => _TVAudioVideoState();
}

class _TVAudioVideoState extends State<TVAudioVideo> {
  final locationController = TextEditingController();
  String currentLocation = "";

  final typeController = TextEditingController(text: "TV, Audio and Video");
  ...
  Widget build(BuildContext context) {
    ...
    _createPetUpload() async {
      var request = http.MultipartRequest('POST', Uri.parse('${httpLinkC}/electronicappliancesupload/'));
      request.fields.addAll({
        'flipperUser': "${sp.uid}",
        'elecType' : "TV, Audio and Video",
        'elecTitle': "${title}",
        'elecDescription': "${description}",
        'elecLocation': "${currentLocation}",
        'elecPrice' : "${price}"
      });
      request.files.add(await http.MultipartFile.fromPath('elecImage', file!.path.toString()));
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => SelectElecApp()),
            );
          },
          icon: const Icon(
