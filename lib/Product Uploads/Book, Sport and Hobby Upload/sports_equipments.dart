...
class SportsEquipments extends StatefulWidget {
  const SportsEquipments({super.key});

  @override
  State<SportsEquipments> createState() => _SportsEquipmentsState();
}

class _SportsEquipmentsState extends State<SportsEquipments> {
  final locationController = TextEditingController();
  String currentLocation = "";

  final typeController = TextEditingController(text: 'Sports Equipments');
  ...
  Widget build(BuildContext context) {
    ...
    _createPetUpload() async {
      var request = http.MultipartRequest('POST', Uri.parse('${httpLinkC}/bookssportshobbiesupload/'));
      request.fields.addAll({
        'flipperUser': "${sp.uid}",
        'bshType' : "Sports Equipments",
        'bshTitle': "${title}",
        'bshDescription': "${description}",
        'bshLocation': "${currentLocation}",
        'bshPrice' : "${price}"
      });
      request.files.add(await http.MultipartFile.fromPath('bshImage', file!.path.toString()));
      ...
      appBar: AppBar(
        ...
        leading: IconButton(
          onPressed: () {
            Navigator.push(
              context,
              MaterialPageRoute(builder: (context) => SelectBSH()),
            );
          },
          icon: const Icon(
